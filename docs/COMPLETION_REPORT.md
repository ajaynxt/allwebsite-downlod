...
- FastAPI analysis API and direct attachment download API
- yt-dlp and FFmpeg integration for best video, resolution choices, MP3, and M4A
- URL safety checks, rights confirmation, rate/resource limits, isolated request processing and immediate cleanup
- Docker, Compose, Nginx, GitHub CI, environment example, tests and operational documents
- `linkdownload.ajaynxt.com` frontend canonical SEO, sitemap, robots, structured data and social preview
...
- GitHub Pages frontend plus VPS/container API because extraction requires a backend, FFmpeg, temporary working space and outbound requests
...
- No user cookies or account credentials; no Cloud Storage/job history; request files are removed after delivery/failure
...
| Direct device download | Implemented | Single attachment response, browser transfer progress and immediate server cleanup |
...
No database, Cloud Storage or migration. Active requests do not survive restart. Rollback uses the previous image/source/config; users restart interrupted downloads.
