...
- Processing/storage: FFmpeg and isolated non-persistent `/tmp`; immediate deletion after response/failure
...
- Logging: application events only; no full URLs, cookies, tokens, or request bodies
- Backup: source/configuration in Git; links and media are never added to backup/storage

## Sensitive assets

| Asset/data | Classification | Stored where | Access | Retention |
|---|---|---|---|---|
| Pasted source URL | User-provided/possibly sensitive | Active request memory only | App process | Request completion/failure |
| Downloaded media | User-provided content | Isolated `/tmp/request-*` directory | Active response only | Deleted immediately after delivery/failure |
...
| `POST /api/download` | URL/mode/quality/permission | None | Strict schema, URL revalidation, allowlisted modes/quality grammar, worker and per-IP limits |
...
| AJX-UP-01/02 | No user upload; generated file isolated per request, served as attachment, then removed | Path and cleanup tests/AJAYNXT |
